...
class DGCNNFeature(collections.namedtuple(
    '_SceneGraphFeature', ('scene_feature', 'object_feature', 'relation_feature', 'multi_relation_feature')
)):
    pass


class DGCNNPointNet(nn.Module):
    def __init__(self, pn_output_dim, output_dim, mode='o'):
        super().__init__()
        ...
        self.graph_encoder = DGCNN(initial_dim=pn_output_dim,
    ...
    def forward(
        ...
        graph_in_features = pn_object_feat.permute(1, 0, 2)
        ...
        for i in range(objects.size(0)):
            ...
            outputs.append(DGCNNFeature(None, this_object_feat, this_relation_feat, this_multi_relation_feat))

...
class SymGraphModelBase(nn.Module):
    @staticmethod
    @def_configs_func
    def _def_configs():
    ...
    def __init__(self, vocab=None):
        ...
        self._def_configs()
        # dgcnn model base
        import models.scene_graph.dgcnn_pointnet as dng
        # dgcnn model + language
        import models.scene_graph.dgcnn_lan_pointnet as dnglan
        pn_output_dim = 128
        output_dim = 607
        self.vocab = vocab
        if self.vocab == None:
            self.scene_graph = dng.DGCNNPointNet(pn_output_dim, output_dim)
        else:
            self.scene_graph = dnglan.DGCNNLanPointNet(pn_output_dim, output_dim, vocab)
    ...
    def forward_sng(self, feed_dict):
        if self.vocab != None:
            f_sng = self.scene_graph(feed_dict.scene, feed_dict.input_objects, feed_dict.input_objects_length, feed_dict.tokens)
        else:
...
class SymGraph3DModel(SymGraphModelBase):
